...
!
        lwrite = .false.
        if ( iargc().lt.4 ) then
            print *,'usage: coordinates2kml lon1|@ lon2|@ lat1|@ lat2|@'
     +           //'[mapfile]'
            call abort
        endif
...
!
        ...
        lookat_range = max(10000.,min(10000000.,
     +       100000*max(abs(lon2-lon1),abs(lat2-lat1))))
!
...
!
        goto 999
 902    write(0,*) 'coordinates2kml: error opening mapfile ',
     +       trim(string)
        call abort
 999    continue
        end
        ...
        return
 901    write(0,*) 'coordinates2kml: error reading coordinate from ',
     +       trim(string)
        call abort
        end
