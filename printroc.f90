...
!   threshold) to construct the ROC curve
    ...
    implicit none
    integer,intent(in) :: ntime,nens
    real,intent(in) :: obs(ntime),fcst(nens,ntime),thobs,pthobs,thmod,pthmod
    real,intent(out) :: area
    logical,intent(in) :: lsame,lprint,lwrite
    integer :: itime,n,iens
    real,allocatable :: prob(:)

    if ( lsame ) then
        call getcutoff(thmod,pthobs,obs,1,1,1,ntime,1,ntime,1,1,0)
    elseif ( pthmod > 0 .and. pthmod < 100 ) then
        call getenscutoff(thmod,pthmod,fcst,1,1,1,ntime &
            ,nens-1,0,nens-1,1,ntime,1,1,0)
        if ( lprint ) then
            write(0,'(a,f6.2,a,g14.6,a)') 'Threshold at ',pthmod,'% corresponds to model ' &
                ,thmod,'<br>'
        endif
    endif
    allocate(prob(ntime))
    do itime=1,ntime
        ...
        n = 0
        do iens=1,nens
            if ( fcst(iens,itime) < 1e30 .and. fcst(iens,itime) /= -999.9 ) then
                n = n + 1
                if ( fcst(iens,itime) > thmod ) then
    ...
    deallocate(prob)
end subroutine probroc

subroutine printroc(obs,fcst,ntime,nens,thobs,pthobs,lprint,lwrite,area)
...
!   Home-grown, I hope fast, algorithm without any binning
    ...
    implicit none
    integer,intent(in) :: ntime,nens
    real,intent(in) :: obs(ntime),fcst(nens,ntime),thobs,pthobs
    real,intent(out) :: area
    logical,intent(in) :: lprint,lwrite
    integer :: i,itime,iens,iabove,nabove,ibelow,nbelow,nhit,nmis,oldhit,oldmis
    real :: old
    real,allocatable :: above(:),below(:)
    character line*80
    logical :: lweb
    integer,save :: noutside=0,nprint=2

!   if needed, convert percentage thobs into aboslute one

    if ( pthobs > 0 .and. pthobs < 100 ) then
        call getcutoff(thobs,pthobs,obs,1,1,1,ntime,1,ntime ,1,1,0)
        if ( lprint ) write(0,'(a,f6.2,a,g14.6,a)') 'Thobs at ' &
            ,pthobs,'% corresponds to observations ',thobs,'<br>'
    endif
    if ( thobs > 1e33 ) then
...
!   that were above the threshold and below the thobs
    ...
    nabove = 0
    do itime=1,ntime
        if ( obs(itime) < 1e30  .and. abs(obs(itime)+999.9) > 0.01 ) then
            if ( obs(itime) < thobs ) then
                do iens=1,nens
                    if ( fcst(iens,itime) < 1e30 .and. abs(fcst(iens,itime)+999.9) > 0.01 ) then
                        nbelow = nbelow + 1
                        below(nbelow) = fcst(iens,itime)
                    endif
                enddo
                if ( lwrite ) print *,'below: ',(fcst(iens,itime),iens=1,min(nens,4))
            else
                do iens=1,nens
                    if ( fcst(iens,itime) < 1e30 .and. abs(fcst(iens,itime)+999.9) > 0.01 ) then
                        nabove = nabove + 1
                ...
                enddo
                if ( lwrite ) print *,'above: ',(fcst(iens,itime),iens=1,min(nens,4))
            endif
        endif
    enddo
    if ( lwrite ) then
    ...
    if ( nbelow == 0 .or. nabove == 0 ) then
        ...
        if ( noutside == 1 ) then
            ...
            write(0,*) 'printroc: cut-off (defined over all data) ',thobs, &
                ' is outside the range observed during the forecast period<br>'
            if ( .false. ) then
                if ( lweb ) then
                    write(0,*) '<table>'
                    do itime=1,ntime
                    ...
                    do itime=1,ntime
                        write(0,*) obs(itime),(fcst(iens,itime),iens=1,nens)
                    enddo
                endif
            endif
        elseif ( noutside >= nprint ) then
            nprint = 2*nprint
            write(0,*) 'printroc: cut-off outside range at ',noutside,' points<br>'
        endif
        area = 3e33
        return
    endif
...
end subroutine printroc

subroutine readthreshold(string,th,pth)
    implicit none
    character,intent(in) :: string*(*)
    real,intent(out) :: th,pth
    integer :: i
    i = index(string,'%')
    if ( i /= 0 ) then
        th = 3e33
