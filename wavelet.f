...
C whatsoever.
*
*       CHANGED: GJvO: added parameter np to give first dimension of wave
      SUBROUTINE WAVELET (n,np,y,dt,mother,param,s0,dj,jtot,npad,
     &			  wave,scale,period,coi)
      IMPLICIT none

      INTEGER n,np,mother,jtot,npad
      DOUBLE PRECISION y(n),dt,param,s0,dj,scale(jtot),period(jtot),
     &	coi(n)
      DOUBLE COMPLEX wave(np,jtot)
...
*GJvO: the data has already been padded by me...
***      DO 20 i=n+1,npad
***	yfft(i) = 0.D0
***20    CONTINUE
...
C*******************************************   Paul wavelet
	...
	norm = SQRT(2.D0*pi*scale1/dt)*
     &	       (2.D0**m/SQRT(m*exp(factln(2*m-1))))
	DO 30 k=1,nk/2+1
...
C whatsoever.
     ...
     &	     ymean,variance,Cdelta,psi0,lwrite)
      IMPLICIT none
     ...
     &	signif(jtot)
      ...
      pi = 4.D0*ATAN(1.D0)
        ...
        if ( lwrite ) then
            print *,'wave_signif: input'
            print *,'isigtest = ',isigtest
            print *,'n,y      = ',n,(y(i),i=1,min(n,10))
            print *,'dt       = ',dt
            print *,'mother   = ',mother
            print *,'param    = ',param
            print *,'dj,jtot  = ',dj,jtot
            print *,'scale    = ',(scale(i),i=1,min(jtot,10))
            print *,'period   = ',(period(i),i=1,min(jtot,10))
            print *,'lag1     = ',lag1
            print *,'siglvl   = ',siglvl
            if ( isigtest.ne.0 ) then
                print *,'dof      = ',dof
            endif
        endif
