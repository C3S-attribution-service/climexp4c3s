subroutine getwinmean(mean,nn,nxmax,nymax,npermax,field,nxf,nyf &
    ,nperyear,yrbeg,yrend,nx,ny,yrbegin,mobegin,nt,x1,x2,y1,y2,lwrite)
...
!   compute means
    ...
    real :: mean(nxmax,nymax,npermax),field(nxf,nyf,nperyear,yrbeg:yrend)
    logical :: lwrite
    integer :: month,year,i,j,k,ii
    integer,external :: normx
    ...
    do k=1,nt
        ...
        if ( .false. .and. lwrite ) print *,'month,year = ',month,year
        do j=y1,y2
            do i=x1,x2
                ii = normx(i,nx)
                if ( field(ii,j,month,year) < 1e33 ) then
...
!   compute mean and variance of a field
    ...
    real :: mean(nxmax,nymax,nzmax,npermax,2),sd(nxmax,nymax,nzmax,npermax), &
...
!   mean
    ...
    do iens=nens1,nens2
        ...
        do it=1,nt
            ...
            if ( .false. .and. lwrite ) print *,'month,year,iens = ',month,year,iens
            do k=1,nz
                do j=y1,y2
                    do i=x1,x2
                        ii = normx(i,nx)
                        if ( field(ii,j,k,month,year,iens) < 1e33 ) then
                            mean(ii,j,k,month,1) = &
                            mean(ii,j,k,month,1) + &
            ...
            do j=1,ny
                do i=1,nx
                    if ( nn(i,j,k,month) > 0 ) then
                        mean(i,j,k,month,1) = mean(i,j,k,month,1)/nn(i,j,k,month)
                    else
...
!   variance
            ...
            do k=1,nz
                do j=y1,y2
                    do i=x1,x2
                        ii = normx(i,nx)
                        if ( field(ii,j,k,month,year,iens) < 1e33 ) then
                            mean(ii,j,k,month,2) = mean(ii,j,k,month,2) &
                                + (field(ii,j,k,month,year,iens) - mean(ii,j,k,month,1))**2
                            nn(ii,j,k,month) = nn(ii,j,k,month) + 1
                        end if
                    end do
                end do
    ...
    do month=1,nperyear
        do k=1,nz
            do j=1,ny
                do i=1,nx
                    if ( nn(i,j,k,month) > 1 ) then
                        mean(i,j,k,month,2) = mean(i,j,k,month,2)/(nn(i,j,k,month)-1)
                    else
...
!   debug info
    ...
    if ( .false. .and. lwrite ) then
        do month=1,nperyear
            ...
            do k=1,nz
                do j=1,ny
                    print '(2i3,1000f10.2)',j,k,(mean(i,j,k,month,1),i=1,nx)
                    print '(2i3,1000f10.2)',j,k,(mean(i,j,k,month,2),i=1,nx)
                    print '(2i3,1000i10)',j,k,(nn(i,j,k,month),i=1,nx)
                end do
            end do
        end do
    end if
end subroutine getenswinmean
