...
!               copy as many whole sets in as fit
                if ( lwrite ) print '(a,i4,i2,a,i4,a,2i4)','At ',i,j,' only found ',mens &
                ...
                do l=1,k-1
                    do jens=1,mens
                        ...
                        if ( lwrite ) print *,'copying member ',validens(jens),' to ', &
                            iens,data(j,i,validens(jens)),data(j,i,iens)
                        ndup(j) = ndup(j) + 1
...
!                   check...
                ...
                310 continue
                    ...
                    k = 1 + int(mens*xran)
                    if ( k <= 0 .or. k > mens .or. validens(k) < 0 ) go to 310
                320 continue
                    if ( data(j,i,iens) < 1e33 ) then
                        iens = iens + 1
                        goto 320
                    endif
                    if ( lwrite ) print *,'copying member ',validens(k),' to ',iens,data(j,i &
