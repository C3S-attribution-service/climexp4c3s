    ...
    !           that year
        ...
        !               first check whether we can make a forecast at all
            ...
            if ( mstart <= month ) then
                ...
                if ( lseries ) then
                    if ( minval(series(mseries,yrf,:)) > 1e33 ) &
                    then
                ...
                if ( lseries ) then
                    if ( minval(series(mseries,yrf-1,:)) > 1e33 ) &
                    then
                        if ( lwrite ) print *,'no series obs', &
                        mseries,yrf-1
                        cycle
                ...
                if ( lseries2 ) then
                    if ( minval(series2(mseries2,yrf-1,:)) > 1e33 ) &
                    then
                        if ( lwrite ) print *,'no series2 obs', &
                        mstart,yrf-1
                        cycle
    ...
    !       fill linear array with data
        ...
        do iens=nens1,nens2
            do yr=yr1,yr2
                ...
                x(y1,iens) < 1e33 ) then
                    if ( abs(x(y1,iens)) > 1e18 .or. &
                    abs(y(yr,iens)) > 1e18 ) then
        ...
        !           and apply the model to make a forecast
            ...
            do iens=nens1,nens2
                do yr=yr1,yr2
                    if ( obs(yr,iens) < 1e33 .and. &
                    obs(yr,iens) > 1e12 ) then
