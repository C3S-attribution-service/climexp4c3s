        ...
        nz1 = 1
        zz1(1) = 0
        call checkgridequal3d(nx,ny,nz,xx,yy,zz,nx1,ny1,nz1,xx1,yy1
     +       ,zz1)
        end

        subroutine checkgridequal3d(nx,ny,nz,xx,yy,zz,nx1,ny1,nz1,xx1
     +       ,yy1,zz1)
        implicit none
        integer nx,ny,nz,nx1,ny1,nz1
        real xx(nx),yy(ny),zz(nz),xx1(nx1),yy1(ny1),zz1(nz1)
        logical lequal
        call aregridsequal3d(nx,ny,nz,xx,yy,zz,nx1,ny1,nz1,xx1,yy1,zz1
     +       ,lequal,.false.)
        if ( .not.lequal ) then
            call aregridsequal3d(nx,ny,nz,xx,yy,zz,nx1,ny1,nz1,xx1,yy1
     +           ,zz1,lequal,.true.)
        ...
        subroutine aregridsequal(nx,ny,xx,yy,nx1,ny1,xx1,yy1,lequal
     +       ,lwrite)
*
...
*
     ...
     +       ,zz1,lequal,lwrite)
        ...
        subroutine aregridsequal3d(nx,ny,nz,xx,yy,zz,nx1,ny1,nz1,xx1,yy1
     +       ,zz1,lequal,lwrite)
*
...
*
        ...
        do i=1,nx
            if ( abs(xx(i)-xx1(i)).gt.0.01 ) then
                lequal = .false.
                if ( lwrite ) print *,'aregridsequal: xx != xx1: ',i,
     +               xx(i),xx1(i)
                return
            endif
        enddo
        do i=1,ny
            if ( abs(yy(i)-yy1(i)).gt.0.01 ) then
                lequal = .false.
                if ( lwrite ) print *,'aregridsequal: yy != yy1: ',i,
     +               yy(i),yy1(i)
                return
            endif
        enddo
        do i=1,nz
            if ( abs(zz(i)-zz1(i)).gt.0.01 ) then
                lequal = .false.
                if ( lwrite ) print *,'aregridsequal: zz != zz1: ',i,
     +               zz(i),zz1(i)
                return
            endif
        enddo
