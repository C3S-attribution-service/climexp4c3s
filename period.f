...
CU    USES avevar
     ...
     *TWOPID
      ...
      nout=0.5*ofac*hifac*n
        if ( nout.gt.np ) then
            write(0,*) 'period: error: output arrays too short'
            write(0,*) '               call with np >= ',nout
            call abort
        endif
        if ( n.gt.NMAX ) then
            write(0,*) 'period: error: internal arrays too small.'
            write(0,*) '               recompile with NMAX >= ',n
            call abort
        endif
        if ( n.le.0 ) then
            write(0,*) 'period: error: no valid data.'
            return
        endif
