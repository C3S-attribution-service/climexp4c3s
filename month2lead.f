...
!
        ...
        do lead=0,11
            do mo=1,12
                do yr=yrbeg,yrend
                    ana = mo - lead
                    if ( ana.le.0 ) ana = ana + 12
                    newdata(mo,yr,lead) = data(mo,yr,ana)                    
                    if ( newdata(mo,yr,lead).lt.1e33 ) then
...
!
        do lead=0,leadmax
            ...
            open(1,file=file)
            do i=1,10
                if ( lines(i)(1:1).eq.'#' ) write(1,'(a)')trim(lines(i))
            enddo
        ...
        do lead=0,leadmax-2
            do yr=yrbeg,yrend
                do mo=1,12
                    ...
                    elseif ( newdata(mo,yr,lead).lt.1e33 .and.
     +                       newdata(mo1,yr1,lead+1).lt.1e33 ) then
