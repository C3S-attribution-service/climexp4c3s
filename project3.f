...
*
        implicit none
        integer nperyear,yr1,yr2,nens1,nens2,nx,ny,nxf,nyf,npermax
     +       ,firstyr,lastyr,month,iens
...
*
        if ( lwrite ) then
            print *,'project: input'
            print *,'nperyear,yr1,yr2,nens1,nens2 = ',nperyear,yr1,yr2
     +           ,nens1,nens2
            print *,'xx,nx,yy,ny = ',(xx(i),i=1,min(3,nx)),'...',xx(nx)
     +           ,nx,(yy(i),i=1,min(3,ny)),'...',yy(ny),ny
        ...
        if ( lwrite ) then
            ...
            do j=ny,1,-1
                print '(i2,15f6.1)',j,
     +               (pattern(i,j,1,1+mod(m1-1,nperyear),yr01),
     +               i=1,min(nx,15))
            end do
            print *,'field = ',(yr1+yr2)/2,m1,nens1
            do j=ny,1,-1
                print '(i2,15f6.1)',j,
     +               (field(i,j,1,1+mod(m1-1,nperyear),
...
*
     ...
     +           ,nperyear*(yr2-yr1+1),.false.)
            do m=m1,m2
                ...
                do k=1,nz
                    do j=1,ny
                        do i=1,nx
                            if ( pattern(i,j,k,month,yr01).lt.1e33 )
     +                           then
                                wt = wt + wx(i)*wy(j)*wz(k)
                                if ( mean(i,j,k,mo).lt.1e33 ) then
     ...
     +                                   pattern(i,j,k,month,yr01)
                ...
                if ( w.gt.minfac*wt ) then
                    avemean(mo) = s/w
                    if ( lwrite ) print *,'avemean(',mo,') = '
     +                   ,avemean(mo)
                else
...
*
     ...
     +                               pattern(i,j,k,month,yr01).lt.1e33 )
     +                               then
                                    wt = wt + wx(i)*wy(j)*wz(k)
     ...
     +                                       *pattern(i,j,k,month,yr01)
                    ...
                    if ( w.gt.minfac*wt .and. avemean(mo).lt.1e33 ) then
                        var(mo,yr,iens) = s/w + avemean(mo)
                        if ( lwrite ) then
                            print *,'var(',mo,yr,iens,') = ',var(mo,yr
     +                           ,iens)
                            print *,'s,w,wt,avemean(mo) = ',s,w,wt
     +                           ,avemean(mo)
                        endif
                    else
                        if ( lwrite ) then
