...
!   signficance
    ...
    include 'params.h'
    include 'getopts.inc'
    integer,parameter :: nvarmax=100,ncuts=9,recfa4=4
...
!   process options
    ...
    if ( string(1:3) == 'all' ) then
        ...
        pcut = pcut/100
        if ( pcut == 1 ) pcut = 0.1 ! choose an arbitrary cut
    end if
...
!       read data
    ...
    do ivar=1,nvars
        ...
        vars(ivar)(1:2) == 'rt2' .or. &
        index(vars(ivar),'_rt') /= 0 ) then
            if ( all == 'all_lo' ) then
                if ( vars(ivar)(1:3) == 'lo_' ) go to 101
            else if ( all == 'all_hi' ) then
                if ( vars(ivar)(1:3) == 'hi_' ) go to 101
            else
                goto 101
            end if
        end if
    end do
    write(*,*) 'fieldsignficance: error: cannot find variable prob*, rt2* or *_rt*'
    goto 999
101 continue
    if ( vars(ivar)(1:4) == 'prob' ) then
        do iivar=1,nvars
            if ( vars(iivar)(1:3) == 'cor' .or. &
                 vars(iivar)(1:3) == 'dif' .or. &
                 vars(iivar)(1:6) == 'reldif' ) go to 102
        end do
        write(*,*) 'fieldsignficance: error: cannot find variable cor*|dif*|reldif*'
        goto 999
    102 continue
    else
        iivar = -999
    end if
    if ( itime <= 0 .or. itime > nt ) then
    ...
    if (  vars(ivar)(1:4) /= 'prob' ) then
        forall(ix=1:nx,iy=1:ny,iz=1:nz, prob(ix,iy,iz) < 1e33) &
    ...
    do icut=1,ncut
        ...
        do iz=1,nz
            do iy=y1,y2
                do ix1=x1,x2
                    ix = ix1
                    if ( xwrap .and. ix < 1 ) ix = ix + nx
                    if ( xwrap .and. ix > nx ) ix = ix - nx
                    if ( abs(prob(ix,iy,iz)/undef-1) > 1e-6 ) then
                        w = wx(ix)*wy(iy)
                        s0 = s0 + w
                        if ( prob(ix,iy,iz) < pcut ) then
                            s = s + w
                        end if
                    end if
                end do
        ...
        if ( .false. .and. p <= pcut ) then
!           save time
            write(0,'(a)') &
                'Statistically, this is indistinguishable from pure chance (p&gt;0.5).'
            goto 999
        end if
!       not enough data...
        if ( x1 == x2 .or. y1 == y2 ) go to 999
        if ( icut == 1 ) then
...
!           it to log(1/p) on 19-06-2013
            ...
            do iz=1,nz
                do iy=1,ny
                    do ix=1,nx
                        if ( prob(ix,iy,iz) < 1e33 ) then
                            if ( prob(ix,iy,iz) < 1e-30 ) then
                                invprob(ix,iy,iz) = 70
                            else
                                invprob(ix,iy,iz) = log(1/prob(ix,iy,iz))
                            end if
                            if ( corr(ix,iy,iz) < 0 ) then
                                invprob(ix,iy,iz) = -invprob(ix,iy,iz)
                            end if
                        end if
                    end do
                end do
...
!           next compute the parameters for getdecor
        ...
        !               and call getcdecor
            ...
            pi = 4*atan(1.)
            deg2rad = atan(1.)/45.
!           decorrelation disk (assuming equal lat/lon decorrelation lengths etc)
            d = pi*decor**2
!           degrees of freedom.  This is not correct if the grid points with
!           valid data are scattered rather than contiguous.
            df = max(1.,area/d)
            if ( .false. .and. df < 2 ) then
                write(0,'(a)') 'This is too small to compute field significance.'
                goto 999
            end if
        end if              ! icut == 1
...
!       estimate field significance
        ...
        do i=1,ncuts
            qcut = cuts(i)/100.
            qq = q(qcut)
            if ( lwrite ) print *,'pcut,qcut,df,q = ',pcut,qcut,df,'=>',qq
            if ( p > qq ) then
                exit
            end if
            qcut1 = qcut
        ...
        if ( ncut == 1 ) then ! normal map
            if ( qcut <= 0.25 ) then
                ...
                if ( lweb ) then
                    write(0,'(a,f5.1,a)') '(p<sub>field</sub> &lt; ',qcut*100,'%).'
                else
                    write(0,'(a,f5.1,a)')'(pfield < ',qcut*100,'%).'
                end if
            else
                write(0,'(2a)') 'Statistically, the map is indistinguishable from random noise'
                write(0,'(a,f5.1,a)') '(pfield > ',100*qcut1,'%).'
            end if
            i = index(infile,'/', .true. )
            j = index(infile,'.', .true. )
            if ( j == 0 ) j = 1 + len_trim(infile)
            if ( lweb ) then
...
!   error messages
    ...
    goto 999
901 write(0,*) 'fieldsignficance: error reading pcut from ',trim(string)
    call exit(-1)
902 write(0,*) 'fieldsignficance: error reading itime from ',trim(string)
    call exit(-1)
904 write(0,*) 'Cannot compute field significance for this grid.'
999 continue
end program fieldsignficance
...
!   get pairs for autocorrelation
    ...
    do j1=1,nyy
        ...
        do i1=1,nxx
            ...
            if ( i <= 0  ) i = i + nx
            if ( i > nx ) i = i - nx
            if ( field(i,j) > 1e33 ) cycle
            if ( abs(field(i,j)) > 1e17 ) then
                itoobig = itoobig + 1
            ...
            do jj1=max(j1,y1),y2
                ...
                if ( jj <= 0  ) jj = jj + ny
                if ( jj > ny ) jj = jj - ny
                if ( abs(yy(j)-yy(jj)) > (nd+1)*d ) cycle
                nii = int(1/cos(yy(jj)*deg2rad))
                do ii1=max(i1,x1),x2,nii
                    ii = ii1
                    if ( ii <= 0  ) ii = ii + nx
                    if ( ii > nx ) ii = ii - nx
                    if ( field(ii,jj) > 1e33 ) cycle
                    if ( abs(field(ii,jj)) > 1e17 ) then
                        jtoobig = jtoobig + 1
                        if ( jtoobig >= mtoobig ) then
                            mtoobig = 2*mtoobig
                            print '(2a,2i5,a,g20.4)', &
                                '# Found too large number 2 ', &
                                'in field(',ii,jj,' = ',field(ii,jj)
                        end if
                        cycle
                    end if
                    lon1 = xx(i)*deg2rad
...
!                   bin
                    ...
                    if ( id < 0 .or. id > nd ) cycle
!                   add to list
                    if ( npairs(id) >= np ) then
...
!       determine distance where it goes through 1/e
    ...
    decor = nd*d            ! default
    do id=0,nd-1
        if ( autocor(id,1) < 1e33 ) then
            do idd=id,nd
                if ( autocor(idd,1) < 1e33 ) then
                    if ( autocor(id,1) > e .and. &
                    autocor(idd,1) <= e ) then
