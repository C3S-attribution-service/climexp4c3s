...
!   should replace repetitive code in lots of other programs
    ...
    character :: file*1023,line*255,tmpvars(1)*60,lvars(1)*100, &
        tmpunits(1)*80,title*512

    if ( lwrite ) then
        print *,'readfield: field has dims ',nxf,nyf,nzf,npermax,fyr,lyr,nens2
        print *,'           ncid    = ',ncid
        print *,'           nens1,2 = ',nens1,nens2
    end if
    if ( firstyr > 10000 ) then
        write(0,*) 'readfield: error while reading ',trim(infile)
        write(0,*) 'readfield: error: wrong value for firstyr ',firstyr
        call exit(-1)
    end if
    call keepalive(0,0)
    ...
    if ( ncid == -1 ) then
        ...
        do iens=nens1,nens2
            ...
            if ( fiyr > 10000 ) then
                write(0,*) 'readfield: error while reading ',trim(infile)
                write(0,*) 'readfield: error: wrong value for fiyr ',fiyr
                call exit(-1)
            end if
