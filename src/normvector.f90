...
!   2: spatial variance is one
    ...
    implicit none
    integer,intent(in) :: nxf,nyf,nzf,nx,ny,nz,n,normalization
    real,intent(inout) :: svdxy(nxf,nyf,nzf,n)
    integer :: i,j,k,ii,n1
    ...
    do ii=1,n
        if ( normalization == 1 ) then
            ...
            write(0,*) 'normsvd1: error: normalization ',normalization,' is not yet supported'
            write(*,*) 'normsvd1: error: normalization ',normalization,' is not yet supported'
            call exit(-1)
        endif
        do j=1,ny
            do i=1,nx
...
!   -2: time series variance is one
    ...
    implicit none
    integer,intent(in) :: nperyear,yr1,yr2,j1,j2,nens1,nens2,neigen, &
        nxf,nyf,nzf,nx,ny,nz,normalization
    real,intent(inout) :: pc(nperyear,yr1:yr2,0:nens2,neigen),svdxy(nxf,nyf,nzf,neigen)
    logical,intent(in) :: lnomissing,lwrite
    ...
    do ii=1,neigen
        if ( normalization == -1 ) then
            ...
            do iens=nens1,nens2
                do y=yr1-1,yr2
                    do m=j1,j2
                        ...
                        if ( yr < yr1 .or. yr > yr2 ) cycle
                        if ( pc(mo,yr,iens,ii) < 1e30 ) then
                            s1 = s1 + (pc(mo,yr,iens,ii) - mean)
                            s2 = s2 + (pc(mo,yr,iens,ii) - mean)**2
                        end if
                    end do
                end do
            end do
            ...
            write(0,*) 'normsvd2: error: normalization ',normalization,' is not yet supported'
            write(*,*) 'normsvd2: error: normalization ',normalization,' is not yet supported'
            call exit(-1)
        end if
        do iens=nens1,nens2
            do y=yr1-1,yr2
                do m=j1,j2
                    ...
                    if ( yr < yr1 .or. yr > yr2 ) cycle
                    if ( pc(mo,yr,iens,ii) < 1e30 ) then
                        pc(mo,yr,iens,ii) = pc(mo,yr,iens,ii)/scale
                    end if
                end do
            end do
        end do
...
!   2: spatial variance is one
    ...
    implicit none
    integer,intent(in) :: nxf,nyf,nx,ny,n,normalization
    real,intent(inout) :: eofxy(nxf,nyf,n)
    integer :: i,j,ii,n1
    real :: xmax,xmin,scale,s1,s2,mean
    ...
    do ii=1,n
        ...
        elseif ( normalization == 2 ) then
            ...
            write(0,*) 'normeof1: error: normalization ',normalization,' is not yet supported'
            write(*,*) 'normeof1: error: normalization ',normalization,' is not yet supported'
            call exit(-1)
        endif
        do j=1,ny
            do i=1,nx
...
!   -2: time series variance is one
    ...
    implicit none
    integer,intent(in) :: nperyear,yr1,yr2,j1,j2,nens1,nens2,neigen,nxf,nyf,nx,ny,normalization
    real,intent(inout) :: pc(nperyear,yr1:yr2,0:nens2,neigen),eofxy(nxf,nyf,neigen)
    logical,intent(in) :: lnomissing,lwrite
    ...
    do ii=1,neigen
        ...
        elseif ( normalization == -2 ) then
            ...
            write(0,*) 'normeof2: error: normalization ',normalization,' is not yet supported'
            write(*,*) 'normeof2: error: normalization ',normalization,' is not yet supported'
            call exit(-1)
        endif
        do iens=nens1,nens2
            do y=yr1-1,yr2
