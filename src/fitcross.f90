subroutine fitcross(x,y,ndata,sig,mwt,a,b,siga,sigb,chi2,q,ncrossvalidate,aa,bb,laabb)
...
!   ncrossvalidate points withheld.
    ...
    implicit none
    integer,intent(in) :: mwt,ndata,ncrossvalidate
    real,intent(in) :: sig(ndata),x(ndata),y(ndata)
    real,intent(out) :: a,b,chi2,q,siga,sigb,aa(ndata),bb(ndata)
    logical,intent(in) :: laabb
...
!   special case causes crashes later on...
    if ( ndata == 1 ) then
        ...
        chi2 = 0
        q = 3e33
        if ( ncrossvalidate > 0 .and. laabb ) bb = 3e33
        return
    endif
    if ( ncrossvalidate >= ndata ) then
        a = 3e33
        b = 3e33
        siga = 3e33
        sigb = 3e33
    ...
    if ( mwt /= 0 ) then
        ...
        do i=1,ndata
            if ( ncrossvalidate == 0 ) then
                ...
                ssx(i) = sx
                ssy(i) = sy
                do j=max(1,i-(ncrossvalidate-1)/2),min(ndata,i+ncrossvalidate/2)
                    wt = 1/sig(j)**2
                    sss(i) = sss(i) - wt
                    ssx(i) = ssx(i) - x(j)*wt
                    ssy(i) = ssy(i) - y(j)*wt
        ...
        if ( ncrossvalidate > 0 .and. laabb ) then
            do j=1,ndata
                do i=1,ndata
                    if ( j < i-(ncrossvalidate)/2 .or. &
                         j > i+(ncrossvalidate-1)/2 ) then
                        t = (x(i) - ax(j))/sig(i)
                        sst2(j) = sst2(j) + t**2
    ...
    if ( st2 == 0 ) then    ! all x-values equal, put b arbitrarily to zero
        b = 0
        sigb = 0
        a = sy/ss
        if ( ncrossvalidate > 0 .and. laabb ) then
