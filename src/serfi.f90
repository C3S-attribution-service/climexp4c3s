...
!>> 1998-11-01 SERFI Krogh  Removed some equivalence for "mangle".
...
!         ERFC(X)/X for 0.0 .LE. X .LE. 0.8.
    ...
    fsign = x
    arg = abs(x)
    if (arg < 0.0e0 .or. arg >= 1.0e0)then
        serfi = 3e33
        return
    end if
    if (arg == 0.0e0) then
