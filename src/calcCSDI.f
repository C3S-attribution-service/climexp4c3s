      subroutine calcCSDI(fyear,lyear,a,qc,p10,b)
c this routine calculates the cold spell duration index
c
c the procedure is:
c 1) to check if T < T_10p  for each day
...
c add the last 5 days of Dec of year i-1 to the array ndum
        ...
        do k=27,31
          nday = nday + 1
          if(qc(i-1,12,k).eq.0) then
            if(a(i-1,12,k).lt.p10(nzhang(i-1),12,k))
     +        ndum(nday) = nprev + 1
          endif
          nprev = ndum(nday)
        enddo
        ...
        do j=1,12
          ...
          do k=1,length
            nday = nday + 1
            if((qc(i,j,k).eq.0).and.(p10(nzhang(i),j,k).gt.absen)) then
              npre = npre + 1
              if(a(i,j,k).lt.p10(nzhang(i),j,k)) 
     +          ndum(nday) = nprev + 1
            endif
            nprev = ndum(nday)
          enddo
...
c add the first 5 days of Jan of year i+1 to the array ndum
        do k=1,6
          nday = nday + 1
          if(qc(i+1,1,k).eq.0) then
            if(a(i+1,1,k).lt.p10(nzhang(i+1),1,k)) 
     +          ndum(nday) = nprev + 1
          endif
          nprev = ndum(nday)
        enddo
