...
*       
     ...
     +       ,yrend,nens1,nens2,nx,ny,nz,yrbegin,mobegin,nt
        ...
        real mean(nxmax,nymax,nzmax,npermax),
     +        field(nxf,nyf,nzf,nperyear,yrbeg:yrend,0:nens2)
        logical lwrite
        ...
        do iens=nens1,nens2
            ...
            do m=1,nt
                ...
                do k=1,nz
                    do j=1,ny
                        do i=1,nx
                            if ( field(i,j,k,month,year,iens).lt.1e33 )
     +                           then
                                mean(i,j,k,month) = mean(i,j,k,month) + 
     +                               field(i,j,k,month,year,iens)
        ...
        if ( lwrite ) then
            do month=1,nperyear
                ...
                do k=1,nz
                    print '(a,i4)','level ',k
                    do j=ny,1,-1
                        print '(i3,1000f8.2)',j,(mean(i,j,k,month),
     +                       i=1,nx)
                        print '(i3,1000i8)',j,(nn(i,j,k,month),
     +                       i=1,nx)
                    end do
                end do
            end do
