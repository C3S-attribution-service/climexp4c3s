        ...
        if ( isvarprcp(var) ) then
            ...
            do iens=nens1,nens2
                do mo=1,nperyear
                    yrzero(mo,iens) = -999
                    do yr=yr1,yr2
                        if ( fcst(mo,yr,iens).le.0 ) then
                            yrzero(mo,iens) = yr
                            exit
                        endif
                    enddo
                enddo
                if ( lwrite ) print *,'debiasmean: taking log10(fcst) '
     +               ,iens
        ...
        do mo=1,nperyear
            ...
            if ( n1.eq.0 .or. n2.eq.0 ) then
                if ( lwrite ) print *,'debiasmean: no valid data for '
     +               ,mo
                do iens=nens1,nens2
                    do yr=yr1,yr2
...
*                   
            do yr=yr1,yr2
                ...
                if ( lwrite ) print *,'debiasmean: bias = ',mo,yr,
     +               ss1,ss2,ss1-ss2
                if ( ss1.lt.1e33 .and. ss2.lt.1e33 ) then
                    do iens=nens1,nens2
                        newfcst(mo,yr,iens) = fcst(mo,yr,iens) + ss1 -
     +                       ss2
                    enddo
                else
                    do iens=nens1,nens2
        ...
        if ( isvarprcp(var) ) then
            ...
            do iens=nens1,nens2
                if ( lwrite ) then
                    ...
                    print *,'yr1,yr2 = ',yr1,yr2
                    do i=yr1,yr2
                        print '(i4,12g12.4)',i,(fcst(j,i,iens),j=1
     +                       ,nperyear)
                    enddo
                endif
                call takeexp(fcst(1,yr1,iens),npermax,nperyear,
     +               yr1,yr2)
                do mo=1,nperyear
                    if ( yrzero(mo,iens).ne.-999 ) then
                        zero = fcst(mo,yrzero(mo,iens),iens)
                        do yr=yr1,yr2
                            if ( fcst(mo,yr,iens).eq.zero ) 
     +                           fcst(mo,yr,iens) = 0
                        enddo
                    endif
                enddo
...
*  #[ debiasvar:
     ...
     +               ,yr1,yr2,nens1,nens2,var,lwrite)
        ...
        real,allocatable :: newfcst(:,:,:),savobs(:,:)
        logical isvarprcp
        integer,save :: init,nunlikely
        data init,nunlikely /0,0/
*
        if ( lwrite ) then
        ...
        if ( isvarprcp(var) ) then
            ...
            do iens=nens1,nens2
                do mo=1,nperyear
                    ...
                    enddo
                enddo
                if ( lwrite ) print *,'debiasvar: taking log10(fcst) '
     +               ,iens
                call takelog(fcst(1,yr1,iens),npermax,nperyear,yr1,yr2)
           enddo
        endif
        do mo=1,nperyear
            ...
            do iens=nens1,nens2
                ...
                enddo
            enddo
            if ( n1.le.2 .or. n2.le.2 ) then
                if ( lwrite ) print *
     +               ,'debiasmean: not enough valid data for ',mo
                do iens=nens1,nens2
                    do yr=yr1,yr2
                        newfcst(mo,yr,iens) = 3e33
                    enddo
            ...
            do iens=nens1,nens2
                do yr=yr1,yr2
                    if ( fcst(mo,yr,iens).lt.1e33 .and. s2m.lt.1e33 )
     +                   then
...
*
     ...
     +                       - 2*fcst(mo,yr,iens)/n2*(fcst(mo,yr,iens)
     +                         - s2m)/n2
     +                       + (n2-1)/real(n2**2)*fcst(mo,yr,iens)**2
                ...
                if ( tt2.lt.0.1*tt1 ) then
                    tt2 = 0.1*tt1
                    if ( init.le.1 ) then
                        init = 2
                        print '(a)','# refusing to inflate variance '//
     +                       'by more than a factor 10'
                    endif
                endif
                do iens=nens1,nens2
                    if ( fcst(mo,yr,iens).lt.1e33 ) then
                        newfcst(mo,yr,iens) = (fcst(mo,yr,iens) - ss2)
     +                       *sqrt(tt1/tt2) + ss1
                        if ( newfcst(mo,yr,iens).gt.1e20 ) then
                            if ( nunlikely.lt.20 ) then
                                nunlikely = nunlikely + 1
                                print *,'debiasvar: unlikely value for '
     +                               //'debiassed forecast '
                                print *,'           s1,s2   = ',s1,s2
                                print *,'           ss1,ss2 = ',ss1,ss2
                                print *,'           t1,t2   = ',t1,t2
                                print *,'           tt1,tt2 = ',tt1,tt2
                                print *,'           fcst(',mo,yr,iens,')
     +                               = ',fcst(mo,yr,iens)
                                print *,'           newfcst(',mo,yr,iens
     +                               ,') = ',newfcst(mo,yr,iens)
                            endif
                        endif
                    else
...
*               copy to working arrays
                if ( yr.gt.yrbeg ) obs1(1:yr-yrbeg) = obs(mo,yrbeg:yr-1)
                if ( yr.lt.yrend ) obs1(yr-yrbeg+1:nyr) = 
     +               obs(mo,yr+1:yrend)
                call nrsort(nyr,obs1)
                do iens=nens1,nens2
                    do i=yrbeg,yr-1
                        fcst1(1+i-yrbeg+nyr*(iens-nens1)) = 
     +                       fcst(mo,i,iens)
                    enddo
                    do i=yr+1,yrend
                        fcst1(i-yrbeg+nyr*(iens-nens1)) =
     +                       fcst(mo,i,iens)
                    enddo
                enddo
...
*  #[ isvarprcp:
        ...
        implicit none
        character var*(*)
        if ( var.eq.'prec' .or. var.eq.'tp' .or. var.eq.'prcp' .or. 
     +       var.eq.'prate' .or. var.eq.'rr' .or. var.eq.'pr' ) then
            isvarprcp = .true.
        else
            isvarprcp = .false.
        endif
