...
*
        integer i,j,k,n,nx,iter,iens,iweird,nweird,ier
        real x,xi,bb(nmc),xixi(nmc),tt(nmc,10),b25
     +       ,b975,xi25,xi975,t5(10),t1(10),db,dxi,f
     +       ,threshold,thens,z,ll,ll1,txtx(nmc),t10(99)
        real ave,adev,sdev,var,skew,curt,y
        character lgt*4
        save iweird,nweird
...
*           as a side effect, data is now sorted
     ...
     +                   exp(xixi(iens)*x*log(10.)))
        ...
        enddo
        do i=1,199
            call getcut(t10(i),real(i)/2,nmc,tt(1,1))
        end do
        call moment(tt(1,1),nmc,ave,adev,sdev,var,skew,curt)
        if ( xyear.lt.1e33 ) then
            call getcut(tx25, 2.5,nmc,txtx)
            call getcut(tx975,97.5,nmc,txtx)
        endif
        do i=1,199
            call merfi(2*i/200.-1,y,ier)
            print *,'^^^',i,t10(i),ave+sdev*y*sqrt(2.)
        end do
*
*       output
*
...
*           preconditioning on xi with gaussian of width restarin/2 around 0
