...
!
     ...
     +       ,0.90,0.95,0.975,1./
        ...
        sw = 0
        do iens=1,nens
            if ( point(iens).lt.1e33 .or. .not.lskipundefined ) then
                n = n + 1
                aa(n) = point(iens)
     ...
     +                   'found 0 ensemble members of iens = ',iens
        ...
        if ( abs(sw-nmod).gt.0.01 .and. .not.lskipundefined ) then
            write(0,*) 'getweightedquant: error: \sum w_n = ',sw,
     +           'but nmod = ',nmod
            call abort
        end if
        if ( n.lt.2 ) then
...
!       quantiles slightly less so
        ...
        do icut=-5,5
            ...
            do i=1,n
                w1 = w
                w = w + ww(ii(i))
                if ( w.gt.(nmod+wn)*cuts(icut) ) then
                    exit
                end if
            end do
            if ( lwrite ) then
                print '(a,f7.3)','found quantile ',cuts(icut)
                if ( i.gt.1 ) print '(i4,3f7.3)',i-1,w1/(nmod+wn),
     +               aa(ii(i-1)),ww(ii(i-1))
                if ( i.le.n ) print '(i4,3f7.3)',i,w/(nmod+wn),
     +               aa(ii(i)),ww(ii(i))
            end if
            if ( i.eq.1 ) then  ! extrapolate - not yet ready
                ...
                if ( abs(aa(ii(i-1))).gt.1e33 .or.
     +               abs(aa(ii(i))).gt.1e33 ) then
