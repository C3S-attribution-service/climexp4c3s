FROM centos/devtoolset-7-toolchain-centos7:7
USER root
WORKDIR /src

MAINTAINER KNMI <info@knmi.nl>

RUN yum update -y && yum install -y \
    epel-release deltarpm

# Install dependencies from repository
RUN yum install -y \
    hdf5 \
    netcdf \
    make \
    netcdf-fortran-devel 

# Install gsl
WORKDIR /src
RUN curl -L "ftp://ftp.gnu.org/gnu/gsl/gsl-2.5.tar.gz" > gsl-2.5.tar.gz
RUN tar -xzvf gsl-2.5.tar.gz 
WORKDIR /src/gsl-2.5
RUN ./configure --prefix /build
RUN make -j4
RUN make install

# Install fgsl
WORKDIR /src
RUN curl -L "https://doku.lrz.de/download/attachments/28051060/fgsl-1.2.0.tar.gz" > fgsl.tar.gz
RUN tar -xzvf fgsl.tar.gz 
ENV PKG_CONFIG_PATH=/build/lib/pkgconfig/
WORKDIR /src/fgsl-1.2.0
RUN ./configure --prefix /build
RUN make
RUN make install
 
# Install climate explorer
WORKDIR /src
COPY . climexp
ENV PVM_ARCH build
WORKDIR /src/climexp/${PVM_ARCH}
COPY ./Docker/Makefile.docker /src/climexp/${PVM_ARCH}/Makefile

CMD bash

#RUN make
