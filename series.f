        program series
*
*       convert a standard .dat file into a text file suitable for
...
*
        ...
        if ( iargc().eq.3 ) then
            lplot = .true.
            call getarg(2,line)
            if ( line(1:4).ne.'plot') then
                write(0,*) 'series: error: expecting "plot", found '
     +               ,trim(line)
                call abort
            end if
            call getarg(3,line)
	...
	if ( index(line,'/dd').gt.0 .or. index(line,'dd') .eq.1 ) then
...
*
        do j=80,1,-1
            if ( line(j:j).eq.'/' ) goto 100
        enddo
  100   continue
        j = j+1
        if ( line(j:j).eq.'p' ) then
            lprec = .TRUE.
     ...
     +        ' Apr-Sep Jan-Dec Jul-Jun; repeated with 10-yr running ',
     +        'average'
        print '(a,101x,a)','#'
     +        ,'(the year is the year of the last month)'
*
*       sum
*
        if ( nperyear.ne.12 ) then
            write(0,*) 'series: unable to handle nperyear != 12 yet '
     +            ,nperyear
            write(0,*) 'Sorry'
            call abort
        endif
...
*
            do j=1,4
               ...
               do iens=mens1,mens
                  if ( j.eq.1 ) then
                     if ( i.eq.yrbeg .or. 
     $                     data(12,max(yrbeg,i-1),iens).gt.1e33 .or. 
     +                     data(1,i,iens).gt.1e33 .or. 
     $                     data(2,i,iens).gt.1e33 ) then
                        s(j+13,i) = s(j+13,i)
                     elseif ( itype.eq.0 ) then
                        s(j+13,i) = s(j+13,i) + data(12,i-1,iens) +
     $                       data(1,i,iens) + data(2,i,iens)
                        n = n + 1
     ...
     +                        cos(a1*data(2,i,iens)))/a1
                     ...
                     if (  data(3*j-3,i,iens).gt.1e33 .or. 
     +                    data(3*j-2,i,iens).gt.1e33 .or. 
     +                    data(3*j-1,i,iens).gt.1e33 ) then
                        s(j+13,i) = s(j+13,i)
                     elseif ( itype.eq.0 ) then
                        s(j+13,i) = s(j+13,i) + data(3*j-3,i,iens) +
     +                       data(3*j-2,i,iens) + data(3*j-1,i,iens)
                        n = n + 1
                     elseif ( itype.eq.1 ) then
                        s(j+13,i) = s(j+13,i) + atan2(
     ...
     +                       cos(a1*data(3*j-1,i,iens)))/a1
               ...
               if ( n.eq.0 ) then
                  s(j+13,i) = 3e33
               elseif ( .not.lprec .and. itype.eq.0 ) then
...
*       
            ...
            do iens=mens1,mens
               if ( i.eq.yrbeg .or. 
     +              data(10,max(yrbeg,i-1),iens).gt.1e33 .or.
     +              data(11,max(yrbeg,i-1),iens).gt.1e33 .or.
     +              data(12,max(yrbeg,i-1),iens).gt.1e33 .or.
     +              data(1,i,iens).gt.1e33 .or.
     +              data(2,i,iens).gt.1e33 .or.
     +              data(3,i,iens).gt.1e33 ) then
                  s(18,i) = s(18,i)
               else
                  if ( itype.eq.0 ) then
                     s(18,i) = s(18,i) + 
     ...
     +                    cos(a1*data(3,i,iens)))/a1
            ...
            s(19,i) = 0
            do iens=mens1,mens
               if ( data(4,i,iens).gt.1e33 .or. 
     $              data(5,i,iens).gt.1e33 .or. 
     $              data(6,i,iens).gt.1e33 .or. 
     $              data(7,i,iens).gt.1e33 .or.
     $              data(8,i,iens).gt.1e33 .or.
     $              data(9,i,iens).gt.1e33 ) then
                  s(19,i) = s(19,i)
               elseif ( itype.eq.0 ) then
                  s(19,i) = s(19,i) + 
     ...
     $                 data(8,i,iens) + data(9,i,iens)
            ...
            if ( .not.lprec .and. itype.eq.0 ) then
               if ( s(18,i) .lt.1e30 ) s(18,i) = s(18,i)/6
               if ( s(19,i) .lt.1e30 ) s(19,i) = s(19,i)/6
            endif
*
*           years
*
               ...
               do iens=mens1,mens
                  if ( itype.eq.0 ) then
                     ...
                     do j=1,6
                        ...
                        if ( data(j+6,i-1,iens).lt.1e33 ) then
     ...
     +                       data(j+6,i-1,iens).lt.1e33 ) then
                           sx = sx + cos(a1*data(j,i,iens)) + 
     ...
     +                          sin(a1*data(j+6,i-1,iens))
            ...
            if ( .not.lprec .and. itype.eq.0 ) then
               if ( s(20,i).lt.1e33 ) s(20,i) = s(20,i)/12
               if ( s(21,i).lt.1e33 ) s(21,i) = s(21,i)/12
            endif
*
*           normalize
*       
...
*
        do i=yrbeg,yrend
            if ( i-5.lt.yrbeg .or. i+5.gt.yrend ) then
                do j=22,41
                    s(j,i) = 3e33
                enddo
            else
                ...
                elseif ( itype.eq.1 ) then
                    do j=2,21
                        ...
                        if ( sx.gt.1e30 ) sx = 3e33
                        do k=-4,4
                            if ( s(j,i-k).gt.1e33 ) then
                                sx = 3e33
                            else
     ...
     +           'were added to let gnuplot make nice plots!'
            do i=yrbeg,yrend
                if ( s(1,i).ge.absent/3 ) then
                    ...
                    linvalid = .false.
                    do j=2,21
                        if ( s(j,i).eq.-999.9 ) then
                            linvalid = .true.
                        end if
                    end do
                    if ( linvalid .and. i.gt.yrbeg ) then ! at least one invalid value - trick
!                       one line with the undefs replaced by the
!                       previous line
                        t = s(:,i)
                        do j=2,21
                            if ( t(j).eq.-999.9 ) t(j) = s(j,i-1)
                        end do
