...
!    but weighing each model equally.
    ...
    include 'params.h'
    include 'netcdf.inc'
    integer,parameter :: nvarmax=20,ntmax=12
    ...
    if ( index(outfile,'CMIP5ext') /= 0 ) lcomputesd = .true. 
    lwrite = .false. ! .true. 
    lstandardunits = .false. 
    nensmin = 3             ! minimum number of ensemble members to base a sd on

    if ( command_argument_count() < 3 ) then
        write(0,*) 'usage: quantiles_field infile1 infile2 ... outfile'
        write(0,*) 'computes a few standard quantiles in a '// &
...
!   read data
    ...
    nens = command_argument_count() - 1
    nskip  = 0
    do iens=1,nens
        call get_command_argument(iens,infile)
        ...
        if ( model == 'modmean' .or. model == 'onemean' ) then
        ...
        i = index(rip,'p')
        p = rip(i+1:)
        if ( model /= oldmodel .or. p /= oldp .or. exp /= oldexp ) &
        then
            nmod = nmod + 1
            nnmod(nmod) = 0
            models(nmod) = model
...
!   write output to file
    ...
    do it=1,nt
        do ivar=1,nvars
            ...
            else if ( lcomputesd .and. ivar == nvars-1 ) then
                call writencslice(ncid,ntvarid,itimeaxis,ntmax &
                    ,ivars(1,ivar),sd(1,1,1,firstmo+it-1),nx,ny,nz &
                    ,nx,ny,nz,it,1)
            else if ( lcomputesd .and. ivar == nvars ) then
                call writencslice(ncid,ntvarid,itimeaxis,ntmax &
                    ,ivars(1,ivar),sdsd(1,1,1,firstmo+it-1), &
                    nx,ny,nz,nx,ny,nz,it,1)
            else
                write(0,*) 'quantiles_field: error: unknown ivar =',ivar
            end if
        end do
    end do
...
!   parse a standard file name
    ...
    if ( type == 'A1' ) then ! CMIP3
        i = index(file,'_144') - 1
        if ( ichar(file(i:i)) >= ichar('0') .and. &
        ichar(file(i:i)) <= ichar('9') .and. &
        ichar(file(i-1:i-1)) >= ichar('0') .and. &
        ichar(file(i-1:i-1)) <= ichar('9') .and. &
        file(i-2:i-2) == '_' ) then
            rip = file(i-1:i-1)
            i = i-3
        else
        ...
        do iz=1,nz
            do iy=1,ny
                do ix=1,nx
                    ...
                    call getweightedquant(point,nnens,nens,nmod,quant,lwrite)
                    do iquant = -6,6
                        quantiles(ix,iy,iz,mo,iquant) = quant(iquant)
                    end do
                end do
            end do
        end do
...
end subroutine getnaturalvariability
...
!   also computes the sd of the sd.
    ...
    real :: sdmod(nx,ny,nz,nperyear,nmod),sd(nx,ny,nz,nperyear),sdsd(nx,ny,nz,nperyear)
    logical :: lwrite
...
subroutine definevariable(nvars,vars,lvars,name,lname)
    implicit none
    integer :: nvars
    character :: vars(nvars+1)*(*),lvars(nvars+1)*(*),name*(*),lname*(*)
    nvars = nvars + 1
    vars(nvars) = name
    lvars(nvars) = lname
