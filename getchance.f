...
*
  ...
  100   continue
        ...
        if ( string(1:1).eq.'#' ) then
            ...
            if ( string.eq.'# logarithmic plot' ) logscale = .TRUE.
            if ( string.eq.'# sqrt plot' ) sqrtscale = .TRUE.
***            write(2,'(a)') string
            goto 100
        endif
...
*
            if ( nx.eq.0 ) print *,'percentiles given x = ',xgiven
            do i=1,npart-1
                yy = perc(i) - a*(xgiven-x0)
*               (should be binary search)
                do j=1,ndata
                    if ( y(ii(j)).gt.yy ) goto 300
                enddo
  300           continue
                if ( j.eq.1 ) then
                    xx(i) = 0
                elseif ( j.gt.ndata ) then
                    xx(i) = 100
                else
