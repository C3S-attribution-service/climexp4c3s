...
*
     ...
     +           ' [anom] [ensanom] [diff [n]] [begin yyyy] [end yyyy] '
     +           //'[xave n]'
            stop
        endif
*
     ...
     +            ,' to ',nens2
        ...
        if ( lwrite ) then
            if ( nperyear.eq.12  ) then
                ...
                print '(a,i2,a,i2)','# lomb: using periods ',j1,' to '
     +               ,j2
            endif
        endif
*
...
*
     ...
     $       ' to equal-area with log(T) x-axis</font></br>'
        fac = 1
        if ( m1.eq.0 ) then
            if ( lsum.gt.1 ) fac = lsum
        else
            if ( lsum.gt.nperyear ) fac = lsum/real(nperyear)
        endif
        call maxquad(xmax,ymax,epx(nint(ofac)),epy(nint(ofac)),
     +        nout-nint(ofac)+1)
        if ( fac.eq.1 .and. avex.eq.1 .and. mens.eq.0 ) then
