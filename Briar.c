...
int main(int argc, char *argv[]) {
  ...
  // and perform Briar score calculations
...
int netcdfdata(int inputloc2, int inputloc3) {
  ...
  float largest, Briar, latval[lat], longval[longi], 
  ...
  for (i=0; i<lat; i++)
	...
	for (m=0; m<categories; m++) 	
	  Briar += (Y[m]-O[m])*(Y[m]-O[m]);	
      }
      if (true) 
	Briar /= (float)(count);
      else 
	Briar = (float)(categories-1);
...
void ncwriter(int inputloc4) {
  ...
  int i, j, lat_dim_id[1], lat_var_id, long_dim_id[1], long_var_id, 
    Briarid, Briardimid[2];
  long start[2], end[2], latlongstart[1], latend[1], longend[1];
  float latitude[lat], longitude[longi], Briarmap[lat*longi];
  ...
  Briardimid[0] = lat_dim_id[0];
  Briardimid[1] = long_dim_id[0];
  Briarid = ncvardef(inputloc4, "Briarscorevalue", NC_FLOAT, 2, Briardimid);
  ...
  ncattput(inputloc4, NC_GLOBAL, "title", NC_CHAR, 13, (void *)"Briarscoremap");
  ncattput(inputloc4, NC_GLOBAL, "history", NC_CHAR, 35, (void *)"Created using the C program Briar.c");
  ncendef(inputloc4);
  ptr = init_result;
  for (i=0; i<lat; i++)
    for (j=0; j<longi; j++) {
