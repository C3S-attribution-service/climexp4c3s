...
*                           demand at least one valid forecast
                            lvalid = .false.
                            do iens=nens1,nens2
                                if ( fcst(mo,yr,iens).lt.1e33 ) lvalid =
     +                               .true.
                            enddo
                        else
                            lvalid = .true.
                        ...
                        do iens=nens1,nens2
                            if ( fcst(mo,yr,iens).gt.1e33 ) then
                                fcst(mo,yr,iens) = -999.9
                            endif
                        enddo
*                       and a valid observation
                        if ( lvalid .and. obs(mo,yr).lt.1e30 ) then
                            n = n + 1
                            yrstart = min(yrstart,yr)
                            yrstop  = max(yrstop,yr)
                            write(10,'(i4,i5,100g14.6)') yr,mo,obs(mo,yr
     +                           ),(fcst(mo,yr,iens),iens=nens1,nens2)
                        endif
                    enddo
                enddo
            enddo
...
*                           and a valid perfect model
                            if ( lvalid .and. fcst(mo,yr,nmodel).lt.1e30
     +                           ) then
                                n = n + 1
                                write(10,'(i4,i5,100g14.6)') yr,mo
     +                               ,fcst(mo,yr,nmodel)
     +                               ,(fcst(mo,yr,iens),iens=nens1
     +                               ,nmodel-1)
     +                               ,(fcst(mo,yr,iens),iens=nmodel+1
     +                               ,nens2)
                            endif
                        enddo
                    enddo
